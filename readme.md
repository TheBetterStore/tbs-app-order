...
```bash
tbs-app-order$ sam local invoke GetOrdersFunction
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
tbs-app-order$ sam local start-api
tbs-app-order$ curl http://localhost:3000/
```
...
```bash
aws cloudformation delete-stack --stack-name tbs-app-order
```
